...
object TreeIcons {
    ...
    private fun registerThemeIcons() {
        // NOTE: no keepBrand set — every language with a theme vector uses it (the hand-tuned
        // BrandIcons for java/kotlin/xml/dart were only needed for mangled generated paths, which
        // MaterialIcons.pathOverrides now fixes). `flutter` has no theme file-vector, so its legacy
        // BrandIcons registration above stays as the fallback.
