...
object DrawableMapping {
    ...
    private fun toUiVectorNode(node: VectorNode): UiVectorNode = when (node) {
        is VectorPath -> UiVectorPath(
            pathData = node.pathData, fillColor = node.fillColor, strokeColor = node.strokeColor,
            strokeWidthVp = node.strokeWidthVp, fillAlpha = node.fillAlpha, strokeAlpha = node.strokeAlpha,
            fillRule = when (node.fillRule) {
                FillRule.EVEN_ODD -> "evenOdd"
                FillRule.NON_ZERO -> "nonZero"
            },
            strokeCap = node.strokeCap.name.lowercase(),
            strokeJoin = node.strokeJoin.name.lowercase(),
            strokeMiter = node.strokeMiter,
        )
        is VectorGroup -> UiVectorGroup(
            children = node.children.map(::toUiVectorNode),
            translateX = node.translateX, translateY = node.translateY,
            scaleX = node.scaleX, scaleY = node.scaleY,
            rotation = node.rotation,
            pivotX = node.pivotX, pivotY = node.pivotY,
